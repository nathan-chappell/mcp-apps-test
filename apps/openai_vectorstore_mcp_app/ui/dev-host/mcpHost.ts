...
function createAuthFetch(authTokenProvider: AuthTokenProvider): typeof fetch {
...
async function connectWithFallback(
  ...
  const authFetch = createAuthFetch(authTokenProvider);
  try {
...
export async function connectToServer(
  ...
  const client = await connectWithFallback(serverUrl, authTokenProvider);
